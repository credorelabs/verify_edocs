...
export const Modal: FunctionComponent<ModalProps> = ({
  ...
  return (
    <>
      {isOverlayVisible && (
        <div
          className={`credore-modal relative flex flex-col overflow-hidden max-w-[640px] w-full ${className}`}
          {...props}
          style={style}
        >
          <div id="header" className="flex flex-nowrap gap-3 sm:gap-4 justify-start items-center p-4 sm:p-6 sm:pb-4">
          ...
          <div id="content" className="px-4 py-4 sm:px-6 flex-1 overflow-y-auto break-words">
            {children}
          </div>
          ...
          {footer && (
            <>
              ...
              <div
                id="footer"
                className={`flex flex-col flex-wrap xs:flex-nowrap xs:flex-row gap-2 content-center items-stretch xs:items-start p-4 pt-4 sm:p-6 sm:pt-4 ${footerClassName}`}
              >
                {footer}
              </div>
            </>
